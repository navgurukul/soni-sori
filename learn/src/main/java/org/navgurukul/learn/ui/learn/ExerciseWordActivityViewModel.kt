...
class ExerciseWordActivityViewModel(
    private val learnRepo: LearnRepoNew,
    corePreferences: CorePreferences,
    private val stringProvider: StringProvider,
    private var courseId: String,
    private val pathwayId: Int,
) : BaseViewModel<ExerciseWordActivityViewEvents, ExerciseActivityWordViewState>(
    ExerciseActivityWordViewState()
) {
                ...
                .collect {
                    ...
                    if (course == null) {
                        ...
                        _viewEvents.postValue(
                            ExerciseWordActivityViewEvents.ShowToast(stringProvider.getString(R.string.error_loading_data))
                        )
                    } else {
    ...
    fun handle(action: ExerciseActivityWordViewActions) {
        when (action) {
            is ExerciseActivityWordViewActions.ExerciseListItemSelected -> onExerciseListItemSelected(
                action.exerciseId
            )
            is ExerciseActivityWordViewActions.ExerciseMarkedCompleted -> onExerciseMarkedCompleted()
            is ExerciseActivityWordViewActions.NextNavigationClicked -> navigate(ExerciseNavigation.NEXT)
            is ExerciseActivityWordViewActions.PrevNavigationClicked -> navigate(ExerciseNavigation.PREV)
            is ExerciseActivityWordViewActions.OnNextCourseClicked -> launchNextCourse(currentCourse.id)
        }
    }
    ...
    private fun onExerciseListItemSelected(
        ...
        _viewEvents.setValue(
            ExerciseWordActivityViewEvents.ShowExerciseFragment(
    ...
    private fun launchNextCourse(currentCourseId: String) {
        ...
        getNextCourse(currentCourseId)?.let {
            ...
            _viewEvents.postValue(
                ExerciseWordActivityViewEvents.FinishActivity
            )
        }
...
enum class ExerciseWordNavigation { PREV, NEXT }

sealed class ExerciseWordActivityViewEvents : ViewEvents {
    ...
    ) : ExerciseWordActivityViewEvents()

    object FinishActivity : ExerciseWordActivityViewEvents()
    class ShowToast(val toastText: String) : ExerciseWordActivityViewEvents()
}

data class ExerciseActivityWordViewState(
...
sealed class ExerciseActivityWordViewActions : ViewModelAction {
    object OnNextCourseClicked : ExerciseActivityWordViewActions()
    object PrevNavigationClicked : ExerciseActivityWordViewActions()
    object NextNavigationClicked : ExerciseActivityWordViewActions()
    data class ExerciseListItemSelected(val exerciseId: String) : ExerciseActivityWordViewActions()
    object ExerciseMarkedCompleted : ExerciseActivityWordViewActions()
}
