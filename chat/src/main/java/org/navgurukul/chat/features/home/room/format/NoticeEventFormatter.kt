    ...
    private fun formatRoomPowerLevels(
        ...
        userIds.forEach { userId ->
            ...
            if (from != to) {
                ...
                val diff = sp.getString(R.string.cancel, userId, fromStr, toStr)
                diffs.add(diff)
            }
        }
        if (diffs.isEmpty()) {
        ...
        return if (event.isSentByCurrentUser()) {
            sp.getString(R.string.cancel, diffStr)
        } else {
            sp.getString(R.string.cancel, disambiguatedDisplayName, diffStr)
        }
    }
        ...
        return if (widgetContent.isActive()) {
            ...
            if (previousWidgetContent?.isActive().orFalse()) {
                if (event.isSentByCurrentUser()) {
                    sp.getString(R.string.cancel, widgetName)
                } else {
                    sp.getString(
                        R.string.cancel,
                ...
                if (event.isSentByCurrentUser()) {
                    ...
                    sp.getString(R.string.cancel, disambiguatedDisplayName, widgetName)
                }
            }
        } else {
...
//            )
    ...
    private fun formatRoomCreateEvent(event: Event): CharSequence? {
        return event.getClearContent().toModel<RoomCreateContent>()
            ...
            ?.let {
                if (event.isSentByCurrentUser()) {
                    sp.getString(R.string.cancel)
                } else {
                    sp.getString(R.string.cancel, it.creator)
                }
            }
    }
    ...
    private fun formatRoomNameEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return if (content.name.isNullOrBlank()) {
            if (event.isSentByCurrentUser()) {
                ...
                sp.getString(R.string.cancel, senderName)
            }
        } else {
            if (event.isSentByCurrentUser()) {
                sp.getString(R.string.cancel, content.name)
            } else {
                sp.getString(R.string.cancel, senderName, content.name)
            }
        }
    }
    ...
    private fun formatRoomTopicEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return if (content.topic.isNullOrEmpty()) {
            ...
            if (event.isSentByCurrentUser()) {
                sp.getString(R.string.cancel, content.topic)
            } else {
                sp.getString(R.string.cancel, senderName, content.topic)
            }
        }
    }
    ...
    private fun formatRoomHistoryVisibilityEvent(event: Event, senderName: String?): Unit? {
...
//        val formattedVisibility = roomHistoryVisibilityFormatter.format(historyVisibility)
        return if (event.isSentByCurrentUser()) {
//            sp.getString(R.string.cancel, formattedVisibility)
        } else {
//            sp.getString(
//                R.string.cancel,
//                senderName,
...
//            )
    ...
    private fun formatRoomCanonicalAliasEvent(event: Event, senderName: String?): String? {
        ...
        return canonicalAlias
            ...
            ?.let {
                if (event.isSentByCurrentUser()) {
                    sp.getString(R.string.cancel, it)
                } else {
                    sp.getString(R.string.cancel, senderName, it)
                }
            }
            ?: if (event.isSentByCurrentUser()) {
                sp.getString(R.string.cancel)
    ...
    private fun buildMembershipNotice(
        ...
        return when (eventContent?.membership) {
            Membership.INVITE -> {
                when {
                    ...
                    event.stateKey == currentUserId ->
                        ...
                        } ?: sp.getString(R.string.cancel, senderDisplayName)
                    event.stateKey.isNullOrEmpty() ->
                        ...
                        if (event.isSentByCurrentUser()) {
                            ...
                            } ?: sp.getString(R.string.cancel, targetDisplayName)
                        } else {
                            eventContent.safeReason?.let { reason ->
                                sp.getString(
                                    R.string.cancel,
            ...
            Membership.JOIN ->
                if (event.isSentByCurrentUser()) {
                    eventContent.safeReason?.let { reason ->
                        sp.getString(R.string.cancel, reason)
                    } ?: sp.getString(R.string.cancel)
                } else {
                    eventContent.safeReason?.let { reason ->
                        sp.getString(
