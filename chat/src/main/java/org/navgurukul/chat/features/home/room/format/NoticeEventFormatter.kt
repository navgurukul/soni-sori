...
class NoticeEventFormatter(
    ...
    fun format(timelineEvent: TimelineEvent): CharSequence? {
        return when (val type = timelineEvent.root.getClearType()) {
            EventType.STATE_ROOM_JOIN_RULES -> formatJoinRulesEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_CREATE -> formatRoomCreateEvent(timelineEvent.root)
            EventType.STATE_ROOM_NAME -> formatRoomNameEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_TOPIC -> formatRoomTopicEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_AVATAR -> formatRoomAvatarEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_MEMBER -> formatRoomMemberEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            ...
            EventType.STATE_ROOM_ALIASES -> formatRoomAliasesEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_CANONICAL_ALIAS -> formatRoomCanonicalAliasEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            ...
            EventType.STATE_ROOM_GUEST_ACCESS -> formatRoomGuestAccessEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_ENCRYPTION -> formatRoomEncryptionEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_WIDGET,
            EventType.STATE_ROOM_WIDGET_LEGACY -> formatWidgetEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_TOMBSTONE -> formatRoomTombstoneEvent(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            EventType.STATE_ROOM_POWER_LEVELS -> formatRoomPowerLevels(
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
            ...
            EventType.CALL_ANSWER -> formatCallEvent(
                type,
                timelineEvent.root,
                timelineEvent.senderInfo.disambiguatedDisplayName
            )
    ...
    private fun formatRoomPowerLevels(
        ...
        val powerLevelsContent: PowerLevelsContent =
            event.getClearContent().toModel() ?: return null
...
//        }
        ...
        return if (widgetContent.isActive()) {
            ...
            if (previousWidgetContent?.isActive().orFalse()) {
                if (event.isSentByCurrentUser()) {
                    sp.getString(R.string.notice_crypto_unable_to_decrypt_friendly, widgetName)
                } else {
                    sp.getString(
                        R.string.notice_member_no_changes,
                ...
                if (event.isSentByCurrentUser()) {
                    sp.getString(R.string.notification_unknown_new_event, widgetName)
                } else {
                    sp.getString(R.string.notice_crypto_unable_to_decrypt_friendly_desc, disambiguatedDisplayName, widgetName)
                }
            }
        } else {
            val widgetName = previousWidgetContent?.getHumanName()
            if (event.isSentByCurrentUser()) {
                sp.getString(R.string.not_implemented, widgetName)
            } else {
                sp.getString(R.string.not_implemented, disambiguatedDisplayName, widgetName)
            }
        }
    }
    ...
    private fun formatRoomCreateEvent(event: Event): CharSequence? {
        return event.getClearContent().toModel<RoomCreateContent>()
            ...
            ?.let {
                if (event.isSentByCurrentUser()) {
                    sp.getString(R.string.not_implemented)
                } else {
                    sp.getString(R.string.not_implemented, it.creator)
                }
            }
    }
    ...
    private fun formatRoomNameEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return if (content.name.isNullOrBlank()) {
            if (event.isSentByCurrentUser()) {
                ...
                sp.getString(R.string.not_implemented, senderName)
            }
        } else {
            if (event.isSentByCurrentUser()) {
                sp.getString(R.string.not_implemented, content.name)
            } else {
                sp.getString(R.string.not_implemented, senderName, content.name)
            }
        }
    }
    ...
    private fun formatRoomTopicEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return if (content.topic.isNullOrEmpty()) {
            ...
            if (event.isSentByCurrentUser()) {
                sp.getString(R.string.not_implemented, content.topic)
            } else {
                sp.getString(R.string.not_implemented, senderName, content.topic)
            }
        }
    }
    ...
    private fun formatRoomHistoryVisibilityEvent(event: Event, senderName: String?): CharSequence? {
        val historyVisibility =
            event.getClearContent().toModel<RoomHistoryVisibilityContent>()?.historyVisibility
                ?: return null

        val formattedVisibility = roomHistoryVisibilityFormatter.format(historyVisibility)
        return if (event.isSentByCurrentUser()) {
            sp.getString(R.string.not_implemented, formattedVisibility)
        } else {
            sp.getString(
    ...
    private fun formatRoomAliasesEvent(event: Event, senderName: String?): String? {
        ...
        return when {
            ...
            addedAliases.isNotEmpty() ->
                if (event.isSentByCurrentUser()) {
                    sp.getQuantityString(
                        R.plurals.membership_changes,
                        addedAliases.size,
                        addedAliases.joinToString()
                    )
                } else {
    ...
    private fun formatRoomCanonicalAliasEvent(event: Event, senderName: String?): String? {
        ...
        return canonicalAlias
            ...
            ?.let {
                if (event.isSentByCurrentUser()) {
                    sp.getString(R.string.not_implemented, it)
                } else {
                    sp.getString(R.string.not_implemented, senderName, it)
                }
            }
            ?: if (event.isSentByCurrentUser()) {
                sp.getString(R.string.not_implemented)
    ...
    private fun buildMembershipNotice(
        ...
        return when (eventContent?.membership) {
            Membership.INVITE -> {
                when {
                    ...
                    event.stateKey == currentUserId ->
                        ...
                        } ?: sp.getString(R.string.not_implemented, senderDisplayName)
                    event.stateKey.isNullOrEmpty() ->
                        ...
                        if (event.isSentByCurrentUser()) {
                            ...
                            } ?: sp.getString(R.string.not_implemented, targetDisplayName)
                        } else {
                            eventContent.safeReason?.let { reason ->
                                sp.getString(
                                    R.string.not_implemented,
            ...
            Membership.JOIN ->
                if (event.isSentByCurrentUser()) {
                    eventContent.safeReason?.let { reason ->
                        sp.getString(R.string.not_implemented, reason)
                    } ?: sp.getString(R.string.not_implemented)
                } else {
                    eventContent.safeReason?.let { reason ->
                        sp.getString(
