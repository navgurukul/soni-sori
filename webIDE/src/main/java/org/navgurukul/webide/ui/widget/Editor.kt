...
import org.navgurukul.webide.databinding.DialogRefactor2Binding
        ...
        if (hasLineNumbers) {
            ...
            for (i in 0 until lineCount) {
                ...
                if (lines[i].toString().endsWith("\n") || i == lineCount - 1) {
                    if (hasLineNumbers) canvas.drawText(
                        (currentLine + 1).toString(),
                        64f,
                        lineBounds.toFloat(),
                        numberPaint!!
                    )
                ...
                if (i == cursorLine) {
                    if (hasLineNumbers) {
                        canvas.drawRect(
                            0f,
                            (10 + lineBounds - lineHeight).toFloat(),
                            72f,
                            (lineBounds + 8).toFloat(),
                            lineShadowPaint
                        )
                    }
                }
            ...
            canvas.drawRect(
                0f,
                (8 + lineBounds - lineHeight).toFloat(),
                width.toFloat(),
                (lineBounds + 12).toFloat(),
                lineShadowPaint
            )
        }
        ...
        override fun onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean {
            when (item.itemId) {
                1 -> {
                    ...
                    val binding = DialogRefactor2Binding.inflate(inflater)
                ...
                2 -> {
                    ...
                    text =
                        text.insert(selectionStart, startComment).insert(selectionEnd, endComment)
                    return true
                }
            }
