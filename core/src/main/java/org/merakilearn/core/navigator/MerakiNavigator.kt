...
class MerakiNavigator(
    ...
    fun openPlayground(context: Context, code: String? = null, isFromCourse: Boolean) {
        startActivity(
            context,
            playgroundModuleNavigator.launchPlaygroundActivity(context, code, isFromCourse),
            false
        )
    }
...
//    }
    ...
    fun launchScratchJrApp(activity: FragmentActivity) {
        if (dynamicFeatureModuleManager.isInstalled(SCRATCH_JR_MODULE_NAME)) {
            scratchJrAppModuleNavigator?.launchScratchJrApp(activity)
        } else {
            val progress = ProgressDialog(activity).apply {
                setCancelable(false)
                setMessage(activity.getString(R.string.installing_module_message))
