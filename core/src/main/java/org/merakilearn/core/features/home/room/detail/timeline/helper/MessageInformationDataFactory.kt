...
package org.merakilearn.core.features.home.room.detail.timeline.helper
 ...
 */
class MessageInformationDataFactory(
        ...
        return MessageInformationData(
            ...
            referencesInfoData = event.annotations?.referencesAggregatedSummary?.let { referencesAggregatedSummary ->
                val verificationState =
                    referencesAggregatedSummary.content.toModel<ReferencesAggregatedContent>()?.verificationState
    ...
    private fun getE2EDecoration(event: TimelineEvent): E2EDecoration {
        ...
        return if (
            ...
            && session.cryptoService().crossSigningService()
                .getUserCrossSigningKeys(event.root.senderId ?: "")?.isTrusted() == true
        ) {
                    ...
                    val sendingDevice = event.root.content
                        ...
                        ?.let { deviceId ->
                            session.cryptoService()
                                .getDeviceInfo(event.root.senderId ?: "", deviceId)
                        }
                    when {
                        sendingDevice == null -> {
