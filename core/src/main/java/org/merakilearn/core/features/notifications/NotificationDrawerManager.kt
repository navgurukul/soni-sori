package org.merakilearn.core.features.notifications
...
class NotificationDrawerManager(
    ...
    private val vectorPreferences: ChatPreferences,
    ...
    private fun refreshNotificationDrawerBg() {
        ...
        synchronized(eventList) {
            ...
            val roomIdToEventMap: MutableMap<String, MutableList<NotifiableMessageEvent>> =
                LinkedHashMap()
            ...
            while (eventIterator.hasNext()) {
                when (val event = eventIterator.next()) {
                    is NotifiableMessageEvent -> {
                        ...
                        if (shouldIgnoreMessageEventInRoom(roomId) || outdatedDetector?.isMessageOutdated(
                                event
                            ) == true
                        ) {
                            // forget this event
            ...
            for ((roomId, events) in roomIdToEventMap) {
                ...
                for (event in events) {
                    ...
                    roomEventGroupInfo.hasNewEvent =
                        roomEventGroupInfo.hasNewEvent || !event.hasBeenDisplayed
            ...
            for (event in invitationEvents) {
                ...
                if (firstTime || !event.hasBeenDisplayed) {
                    val notification =
                        notificationUtils.buildRoomInvitationNotification(event, session.myUserId)
                    notificationUtils.showNotificationMessage(
                        event.roomId,
            ...
            for (event in simpleEvents) {
                ...
                if (firstTime || !event.hasBeenDisplayed) {
                    val notification =
                        notificationUtils.buildSimpleEventNotification(event, session.myUserId)
                    notificationUtils.showNotificationMessage(
                        event.eventId,
                        ROOM_EVENT_NOTIFICATION_ID,
                        notification
            ...
            if (eventList.isEmpty() || eventList.all { it.isRedacted }) {
                ...
                val sumTitle = stringProvider.getQuantityString(
            ...
            if (file.exists()) {
                FileInputStream(file).use {
                    val events: ArrayList<NotifiableEvent>? =
                        currentSession?.loadSecureSecret(it, KEY_ALIAS_SECRET_STORAGE)
                    if (events != null) {
