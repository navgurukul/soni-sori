...
abstract class AttachmentViewerActivity : AppCompatActivity(),
    org.merakilearn.core.features.attachmentviewer.AttachmentEventListener {
...
//                || !isAtStartPosition
    ...
    fun setSourceProvider(sourceProvider: _root_ide_package_.org.merakilearn.core.features.attachmentviewer.AttachmentSourceProvider) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_attachment_viewer )
        mBinding.attachmentPager.orientation = ViewPager2.ORIENTATION_HORIZONTAL
        attachmentsAdapter = AttachmentsAdapter()
        mBinding.attachmentPager.adapter = attachmentsAdapter
        imageTransitionView = mBinding.transitionImageView
        transitionImageContainer = findViewById(R.id.transitionImageContainer)
        pager2 = mBinding.attachmentPager
        directionDetector = createSwipeDirectionDetector()
        gestureDetector = createGestureDetector()

        mBinding.attachmentPager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(mBinding.rootContainer) { _, insets ->
...
//            Log.v("ATTACHEMENTS", "dispatch to pager")
            return mBinding.attachmentPager.dispatchTouchEvent(ev)
        }
...
//        Log.v("ATTACHEMENTS", "handleTouchIfNotScaled $event")
        ...
        return when (swipeDirection) {
            SwipeDirection.Up, SwipeDirection.Down -> {
                if (isSwipeToDismissAllowed && !wasScaled && isImagePagerIdle) {
                    swipeDismissHandler.onTouch(mBinding.rootContainer, event)
                } else true
            }
            SwipeDirection.Left, SwipeDirection.Right -> {
                mBinding.attachmentPager.dispatchTouchEvent(event)
            }
            else                                      -> true
        }
    }
    ...
    private fun createSwipeToDismissHandler()
            ...
            swipeView = mBinding.dismissContainer,
    ...
    override fun onEvent(event: org.merakilearn.core.features.attachmentviewer.AttachmentEvents) {
        if (overlayView is org.merakilearn.core.features.attachmentviewer.AttachmentEventListener) {
            (overlayView as? org.merakilearn.core.features.attachmentviewer.AttachmentEventListener)?.onEvent(event)
        }
    }
    ...
    fun handle(commands: org.merakilearn.core.features.attachmentviewer.AttachmentCommands) {
