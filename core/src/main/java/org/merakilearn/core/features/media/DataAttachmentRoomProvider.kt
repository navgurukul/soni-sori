...
class DataAttachmentRoomProvider(
    ...
    override fun getAttachmentInfoAt(position: Int): _root_ide_package_.org.merakilearn.core.features.attachmentviewer.AttachmentInfo {
        return attachments[position].let {
            when (it) {
                is ImageContentRenderer.Data -> {
                    if (it.mimeType == "image/gif") {
                        _root_ide_package_.org.merakilearn.core.features.attachmentviewer.AttachmentInfo.AnimatedImage(
                                uid = it.eventId,
                                url = it.url ?: "",
                                data = it
                        )
                    } else {
                        _root_ide_package_.org.merakilearn.core.features.attachmentviewer.AttachmentInfo.Image(
                                uid = it.eventId,
                                url = it.url ?: "",
                                data = it
                        )
                ...
                is VideoContentRenderer.Data -> {
                    _root_ide_package_.org.merakilearn.core.features.attachmentviewer.AttachmentInfo.Video(
                            uid = it.eventId,
                            url = it.url ?: "",
                            data = it,
                            thumbnail = _root_ide_package_.org.merakilearn.core.features.attachmentviewer.AttachmentInfo.Image(
                                    uid = it.eventId,
