package org.merakilearn.core.features.home.room.detail.timeline.factory
...
class MergedHeaderItemFactory(
    ...
    fun create(
        ...
        return if (shouldMergedAsCannotDecryptGroup(event, nextEvent)) {
            ...
            buildUTDMergedSummary(
                currentPosition,
                items,
                event,
                eventIdToHighlight, /*requestModelBuild,*/
                callback
            )
        } else if (nextEvent?.root?.getClearType() == EventType.STATE_ROOM_CREATE
            && event.isRoomConfiguration(
                nextEvent.root.getClearContent()?.toModel<RoomCreateContent>()?.creator
            )
        ) {
    ...
    private fun buildMembershipEventsMergedSummary(
        ...
        return if (prevSameTypeEvents.isEmpty()) {
            ...
            val previousCollapseStateKey =
                mergedEventIds.intersect(mergeItemCollapseStates.keys).firstOrNull()
            val initialCollapseState = mergeItemCollapseStates.remove(previousCollapseStateKey)
                ?: true
            val isCollapsed =
                mergeItemCollapseStates.getOrPut(event.localId) { initialCollapseState }
            if (isCollapsed) {
            ...
            MergedMembershipEventsItem_()
                ...
                .also {
                    it.setOnVisibilityStateChanged(
                        MergedTimelineEventVisibilityStateChangedListener(
                            callback,
                            mergedEvents
                        )
                    )
    ...
    private fun shouldMergedAsCannotDecryptGroup(
        event: TimelineEvent,
        nextEvent: TimelineEvent?
    ): Boolean {
        if (!chatPreferences.mergeUTDinTimeline()) return false
    ...
    private fun buildUTDMergedSummary(
        currentPosition: Int,
        items: List<TimelineEvent>,
        event: TimelineEvent,
        eventIdToHighlight: String?,
    ...
    private fun buildRoomCreationMergedSummary(
        ...
        while (prevEvent != null && prevEvent.isRoomConfiguration(null)) {
            if (prevEvent.root.getClearType() == EventType.STATE_ROOM_ENCRYPTION) {
                hasEncryption = true
                encryptionAlgorithm =
                    prevEvent.root.getClearContent()?.toModel<EncryptionEventContent>()?.algorithm
            }
            mergedEvents.add(prevEvent)
            tmpPos--
