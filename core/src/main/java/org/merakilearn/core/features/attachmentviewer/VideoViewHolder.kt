...
class VideoViewHolder constructor(itemView: View) :
    ...
    var eventListener: WeakReference<org.merakilearn.core.features.attachmentviewer.AttachmentEventListener>? = null
...
//                        Log.v("FOO", "isPlaying $isPlaying $progress/$duration")
                        eventListener?.get()?.onEvent(org.merakilearn.core.features.attachmentviewer.AttachmentEvents.VideoEvent(isPlaying, progress, duration))
                    }
        }
        try {
    ...
    override fun handleCommand(commands: org.merakilearn.core.features.attachmentviewer.AttachmentCommands) {
        if (!isSelected) return
        when (commands) {
            org.merakilearn.core.features.attachmentviewer.AttachmentCommands.StartVideo -> {
                wasPaused = false
                videoView.start()
            }
            org.merakilearn.core.features.attachmentviewer.AttachmentCommands.PauseVideo -> {
                wasPaused = true
                videoView.pause()
            }
            is org.merakilearn.core.features.attachmentviewer.AttachmentCommands.SeekTo  -> {
                val duration = videoView.duration
                if (duration > 0) {
    ...
    override fun bind(attachmentInfo: _root_ide_package_.org.merakilearn.core.features.attachmentviewer.AttachmentInfo) {
