package org.merakilearn.core.features.media
...
import org.merakilearn.core.features.attachmentviewer.AttachmentCommands
import org.navgurukul.chat.features.attachmentviewer.AttachmentViewerActivity
import timber.log.Timber

class MerakiAttachmentViewerActivity : AttachmentViewerActivity(), BaseAttachmentProvider.InteractionListener {
    ...
    private val dataSourceFactory: AttachmentProviderFactory by inject()

    private val imageContentRenderer: ImageContentRenderer by inject()
    ...
    companion object {
        ...
        fun newIntent(context: Context,
                      ...
                      sharedTransitionName: String?) = Intent(context, MerakiAttachmentViewerActivity::class.java).also {
    ...
    override fun onPlayPause(play: Boolean) {
        handle(if (play) org.merakilearn.core.features.attachmentviewer.AttachmentCommands.StartVideo else org.merakilearn.core.features.attachmentviewer.AttachmentCommands.PauseVideo)
    }

    override fun videoSeekTo(percent: Int) {
        handle(org.merakilearn.core.features.attachmentviewer.AttachmentCommands.SeekTo(percent))
    }
    ...
    override fun onShareTapped() {
        this.currentSourceProvider?.getFileForSharing(currentPosition) { data ->
            if (data != null && lifecycle.currentState.isAtLeast(Lifecycle.State.RESUMED)) {
                shareMedia(this@MerakiAttachmentViewerActivity, data, getMimeTypeFromUri(this@MerakiAttachmentViewerActivity, data.toUri()))
            }
        }
    }
}