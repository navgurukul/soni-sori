...
const getExtensionURLOfThisVersion = require('../extension-support/previous-extensions-support.js').getExtensionURLOfThisVersion;
...
/**
 * Returns the block name removing the extension ID from the opcode.
 * @param {!string} opcode The opcode to examine for extension.
 * @return {?string} The block name, if it exists and is not a core extension.
 */
const getExtensionBlockName = function (opcode) {
    const index = opcode.indexOf('_');
    const prefix = opcode.substring(index + 1);
    if (prefix !== '') return prefix;
}
...
const serializeCostume = function (costume) {
    ...
    // This will convert 0.5 or any floating number to 1
    obj.bitmapResolution =
        typeof costume.bitmapResolution == 'undefined'
            || (isFloat_number(costume.bitmapResolution) || costume.bitmapResolution < 0)
            ? 1 : costume.bitmapResolution;
...
const serializeVariables = function (variables) {
    ...
    obj.imageVars = Object.create(null);
    let varIdList = [];
    for (const varId in variables) {
        if (varIdList.includes(varId)) continue;
        ...
        if (v.type === Variable.IMAGE_TYPE) {
            obj.imageVars[varId] = v.value;
            continue;
        }
        ...
        if (v.dataType) {
            obj.variables[varId] = [v.name + `${v.dataType.toLowerCase() === 'string' ? '_string' : '_float'}`, v.value];
        } else
 ...
 */
const serialize = function (runtime, targetId, saveData) {
    // console.log(runtime)
    // Fetch targets
    const obj = Object.create(null);
    ...
    // Variables might exploit the condition of having one isStage with true condition
    const variablesOnStage = [];
    const filteredFlattenedOriginalTargets = flattenedOriginalTargets.filter(t => {
        let shouldSerialize = true;
        let variableIdList = Object.keys(t.variables);
        variableIdList.forEach(variableId => {
            variablesOnStage.forEach(variableOnStage => {
                if (variableOnStage.variableId == variableId && variableOnStage.isStage == t.isStage) {
                    shouldSerialize = false;
                }
            });
            if (shouldSerialize) {
                variablesOnStage.push({ variableId: variableId, isStage: t.isStage });
            }
        });
    ...
    if (runtime.renderer && !targetId) {
        filteredFlattenedOriginalTargets.forEach((t, index) => {
            t.layerOrder = layerOrdering[index];
        });
        runtime.emitProjectChanged();
    }

    const serializedTargets = filteredFlattenedOriginalTargets.map(t => serializeTarget(t, extensions));
    ...
    //Added support for storing teachableMachine Model in saved sb3 file
    if (obj.extensions.includes('teachableMachine')) {
        let extensionDispatchId = centralDispatch.services.extensions._loadedExtensions.get('teachableMachine');
        obj.teachableMachineModelUrl = centralDispatch.services[extensionDispatchId].modelUrl;
        obj.teachableMachineModelType = centralDispatch.services[extensionDispatchId].modelType;
    }
    if (obj.extensions.includes('faceDetection')) {
        console.log("CHECK B12", saveData);
        if (saveData === true) {
            let extensionDispatchId = centralDispatch.services.extensions._loadedExtensions.get('faceDetection');
            obj.faceComparision = centralDispatch.services[extensionDispatchId].toJSON();
        }
    }
    // Assemble metadata
    ...
    // Attach board selected
    obj.boardSelected = runtime.boardSelected;
    obj.pictobloxVersion = runtime.pictobloxVersion;
    return obj;
};
...
const deserializeFields = function (fields) {
    ...
    for (const fieldName in fields) {
        ...
        } else if (fieldName === 'IMAGE_OPTION') {
            obj[fieldName].variableType = Variable.IMAGE_TYPE;
...
const parseScratchObject = function (object, runtime, extensions, zip, assets) {
    ...
    if (object.hasOwnProperty('blocks')) {
        ...
        for (const blockId in object.blocks) {
            ...
            const blockName = getExtensionBlockName(blockJSON.opcode);
            if (extensionID) {
                let newExtensionID = getExtensionURLOfThisVersion(extensionID).extension;
                if (newExtensionID !== extensionID) {
                    blockJSON.opcode = newExtensionID + "_" + blockName;
                }
            }
    ...
    if (object.hasOwnProperty('variables')) {
        for (const varId in object.variables) {
            ...
            if (variable[0].endsWith('_string')) {
                newVariable = new Variable(
                    varId, // var id is the index of the variable desc array in the variables obj
                    variable[0].substring(0, variable[0].length - '_string'.length), // name of the variable
                    Variable.SCALAR_TYPE, // type of the variable
                    isCloud,
                    'string'
                );
            } else if (variable[0].endsWith('_float')) {
                newVariable = new Variable(
                    varId, // var id is the index of the variable desc array in the variables obj
                    variable[0].substring(0, variable[0].length - '_float'.length), // name of the variable
                    Variable.SCALAR_TYPE, // type of the variable
                    isCloud,
                    'float'
                );
    ...
    if (object.hasOwnProperty('imageVars')) {
        for (const imageId in object.imageVars) {
            const imageVar = object.imageVars[imageId];
            const newImageVar = new Variable(
                imageId,
                imageVar,
                Variable.IMAGE_TYPE,
                false
            ...
            // no need to explicitly set the value, variable constructor
            // sets the value to the same as the name for image vars
            target.variables[newImageVar.id] = newImageVar;
        }
    }
