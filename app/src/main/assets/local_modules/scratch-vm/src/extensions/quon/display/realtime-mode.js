const displayImage = (args, util, parent) => {
    const arg = [util.imageToRgb(args.IMAGE, args.WIDTH, args.HEIGHT), args.X_AXIS, args.Y_AXIS, args.WIDTH, args.HEIGHT]
    // util.runtime.writeToPeripheral(arg,,2);
    return util.setSendDelay();
}

const enableHeaderBlock = (args, util, parent) => {
    console.log(`From realtimeMode: ${args}`);
}

const colourMatrix = (args, util, parent) => {
    const arg = args.MATRIX
    // util.runtime.writeToPeripheral(arg,,2)
    return util.setSendDelay();
}
...
const rgbColor = (args, util, parent) => {
    const arg = util.rgbToHex(args.RED, args.GREEN, args.BLUE);
    // util.runtime.writeToPeripheral(arg,,2)
    return util.setSendDelay();
}
...
const write = (args, util, parent) => {
    ...
    console.log("arg: ",arg," ",arg.map(ele => ele.charCodeAt(0)));
