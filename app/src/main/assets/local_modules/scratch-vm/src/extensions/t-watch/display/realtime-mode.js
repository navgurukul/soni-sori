const setImage = (args, util, parent) => {
    return;
}
...
const displayImagefromSD = ( args, util, parent) => {
    return;
}
...
const setScreenRotation = (args, util, parent) => {
    const arg = args.ROTATION_VALUE;
    util.runtime.writeToPeripheral(arg, 124, 2);
    return util.setSendDelay();
};
...
const getTouchValue = (args, util, parent) => {
    const dataBuffer = (args.AXIS === 'X' ? `1` : `2`);
    return new Promise(resolve => {
        util.runtime.writeToPeripheral(dataBuffer, 123, 1);
