...
const { OrderedMap } = require('../../../immutable/dist/immutable.js');
const axios = require('../../../axios/index');
const keyMirror = require('../../../keymirror/index.js');
...
const isOnline = require( "../../../scratch-vm/local_modules/is-online/browser.js");
...
const { BlockNode, BlockInput } = require('../util/blockNode');
...
const SerialCom = require('../util/serialCom');

const defaultExtensionColors = ['#0FBD8C', '#0DA57A', '#0B8E69'];

const ImageOverlay = require('../image-overlay/imageOverlay');

const boards = require('../util/board-config').default;

const { boardMap, ModelLoadingType } = require('../util/board-config');

// const VMPreSerialStoreProcedureState = keyMirror({
//     STARTED: null,
//     STOPPED: null,
//     TERMINATED: null
// });

const PICTOBLOX_VERSION = 'm.3.6.0';
...
const ArgumentTypeMap = (() => {
    ...
    map[ArgumentType.MATRIX2] = {
        shadowType: 'matrix2',
        fieldType: 'MATRIX'
    };
    map[ArgumentType.MATRIX3] = {
        shadowType: 'matrix3',
        fieldType: 'MATRIX'
    };
    map[ArgumentType.MATRIXLEFT] = {
        shadowType: 'matrix4_left',
        fieldType: 'MATRIX'
    };
    map[ArgumentType.MATRIXRIGHT] = {
        shadowType: 'matrix4_right',
        fieldType: 'MATRIX'
    };
    map[ArgumentType.MATRIXCOLOUR] = {
        shadowType: 'matrixColour',
        fieldType: 'MATRIX'
    };
    map[ArgumentType.MATRIXCOLOUR5X7] = {
        shadowType: 'matrixColour5x7',
        fieldType: 'MATRIX'
    };
    map[ArgumentType.MATHSLIDER255] = {
        shadowType: 'math_slider_0_255',
        fieldType: 'NUM'
    };
    map[ArgumentType.MATHSLIDER100] = {
        shadowType: 'math_slider_0_100',
        fieldType: 'NUM'
    };
    map[ArgumentType.MATHSLIDER159] = {
        shadowType: 'math_slider_0_159',
        fieldType: 'NUM'
    };
    map[ArgumentType.MATHSLIDER127] = {
        shadowType: 'math_slider_0_127',
        fieldType: 'NUM'
    };
    map[ArgumentType.MATHSLIDER180] = {
        shadowType: 'math_slider_0_180',
        fieldType: 'NUM'
    };
    map[ArgumentType.MATHSLIDER200] = {
        shadowType: 'math_slider_0_200',
        fieldType: 'NUM'
    };
    map[ArgumentType.MATHSLIDER320] = {
        shadowType: 'math_slider_0_319',
        fieldType: 'NUM'
    };
    map[ArgumentType.MATHSLIDER200] = {
        shadowType: 'math_slider_0_20',
        fieldType: 'NUM'
    };
    map[ArgumentType.IMAGE] = {
        shadowType: 'imageDropdown',
        fieldType: 'TEXT'
    };
    map[ArgumentType.IMAGE_SELECT] = {
        shadowType: 'imageSelect'
    };
    map[ArgumentType.IMAGE_SELECT_240x240] = {
        shadowType: 'imageSelect240x240'
    }
    map[ArgumentType.IMAGE_MENU] = {
        shadowType: 'imageMenu'
    };
...
class Runtime extends EventEmitter {
    constructor() {
        ...
        /**
         * Map to look up all block information by extended opcode on workspace before refreshing.
         * @type {Array.<CategoryInfo>}
         * @private
         */
        this._oldBlockInfo = [];
        ...
        /**
         * Current reference of a input.
         * Changes as user focuses on other input and when user gets blur on current input
         * WORK_TIME.
         * @type {HTMLInputElement}
         */
        this.currentInputRef = null;

        this.isVMStoreRuning = false;

        this.getCode = false;
        this.serialValue = 0;
        this.boardSelected = boards.None;
        this.pictobloxVersion = PICTOBLOX_VERSION;
        /**
         * Added For AI extension block to transger data from GUI to VM
         */
        this.aiBlockDataFromGUI = null;

        /**
        * Default Hardware Extensions
        */
        this.defaultHardwareExtensions = {
            [boards.None]: [
            ],
            [boards.Evive]: [
                'evive',
        ...
        /**
         * Boards allowed for VM Pre Serial Store Procedure
         * Store from boards because comparision will happen with currentBoard
         */
        this.vmPreStoreProcedureAllowedBoards = [
            boards.Quarky
        ];

        /**
         * ExtentionURL in this object will be loaded in the background.
         * Do not forget to remove the extentionURL in this object from this.runtime.defaultHardwareExtensions object.
         */
        this.backgorundHardwareExtensions = {
            'all': [
                'dabble',
                'communication',
        ...
        /**
         * ExtentionURLs which are common in each board.
         */
        this.commonHardwareExtensions = [
            'actuators',
            'sensors',
            'dabble',
            'communication',
        ...
        /**
        * Extensions which requires sign-in
        */
        this.signInRequiredExtensions = [
            'aiServices',
            'teachableMachine',
            'objectDetection',
            'posenet',
        ...
        /**
         * Extensions provided by Scratch
         */
        this.scratchExtensions = [
            'music',
            'pen',
            'videoSensing',
            'text2speech',
        ...
        /**
         * Extensions which gives notice when loaded
         */
        this.showNoticeOnExtensionLoad = [
            'aiServices'
        ];

        /**
         * Hide 'Upload Firmware' button on StageHeader
         */
        this.hideUploadFirmwareForBoard = [
            boardMap["micro:bit"]
        ];
         ...
         * Store from boards because comparision will happen with currentBoard
         */
        this.vmPreSerialStoreProcedureAllowedBoards = [
            boards.Quarky
        ];

        /**
         * Stores the blocks which are not usable or which has isUsable set to false.
         * @type {Array}
         */
        this.notUsableBlocks = [];

        /**
         * Stores the blocks which are from common extension and are not usable in another board
         * @type {Object}
         */
        this.boardSpecificExtraBlocksFromCommonExtensionObj = {};
         ...
         * @type {Object}
         */
        this.boardSpecificBlocksFromCommonExtensionObj = {};

        /**
         * Stores common extension name for boards.
         * @type {Object}
         */
        this.commonExtensionAmongBoards = {};

        /**
         * Stores common extension id, loaded from extension library.
         * @type {Object}
         */
        this.commonExtensionLoadedFromExtensionLibrary = [];
        ...
        /**
         * Creates a list of common extensions among boards from commonHardwareExtensions.
         */
        ...
        /**
         * The times user has user AI Services
         */
        this.aiServiceCount = 0;

        /**
         * Video Input Device position because deviceId will change at every component refresh
         */
        this.videoInputDevicePosition = 0;

        /**
         * VideoProvide Instance Container
         */
        this.videoProviderContainer = [];


        /**
         * The pin for which VMPreSerialStore procedure is going to start or stop
         */
        this.pinSelectedForVMPreSerialStore = 2;

        /**
         * Stores the serial read data
         */
        this.VMPreSerialStore = {};

        /**
         * Represents the state of VMPreSerialStore procedure
         */
        // this.vmPreSerialStoreProcedureState = VMPreSerialStoreProcedureState.TERMINATED;

        /**
         * Represents the state of serial write queue handling procedure
         */
        // this.serialWriteQueueProcedureState = VMPreSerialStoreProcedureState.TERMINATED;

        /**
         * Serial Write Queue
         */
        this.serialWriteQueue = [];

        /**
         * Represents the number of times VM Pre Serial Store Procedure Executed
         */
        this.timesVMPreSerialStoreProcedureExecuted = 0;

        /**
         * Represents the value of timeout for restarting of VM Pre Serial Store Procedure
         */
        this.vmPreSerialStoreRestartTimeoutValue = 10;

        /**
         * Represents the value of interval to keep checking Serial Write Queue
         */
        this.serialWriteQueueIntervalValue = 5;

        /**
         * Represents the timeout of VM Pre Serial Store Procedure
         */
        this.vmPreSerialStoreRestartTimeout = null;

        /**
         * Represents the interval of Serial Write Queue procedures
         */
        this.serialWriteQueueInterval = null;

        /**
         * The ModelLoadingType which is being loaded at the time
         */
        this.modelLoadingType = null;

        /**
         * The Model Loading Id to identify the listener when the modelData received from Mobile Middleware
         */
        this.modelLoadingId = null;

        /**
         * Upload firmware first case modal for the boards which can upload firmware using Mobile App.
         * The boards which are not in the first case are automatically added in the second case.
         */
        this.uploadFirmwareAvailableBoards = [
            boards.Evive,
            boards.ArduinoUno,
            boards.ArduinoMega,
            boards.ArduinoNano
        ...
        /**
         * Data consent needed if below any extension
        */
        this.dataConsentNeededForExtensions = [
            'faceDetection'
        ];

        this.firmwareVer = 2;  //Update the latest version here
    }
    ...
    /**
     * Event name when sign-in is required
     * @const {string}
     */
    static get SIGN_IN_REQUIRED_ERROR() {
        return 'SIGN_IN_REQUIRED_ERROR';
    }

    /**
    * Event to indicate that the user don't have credits required to make a successful request
    * @const {string}
    */
    static get INSUFFICIENT_CREDITS() {
        return 'INSUFFICIENT_CREDITS';
    }

    /**
     * Event to indicate to scroll to the particular extension
     * @const {string}
     */
    static get SCROLL_TO_EXTENSION() {
        return 'SCROLL_TO_EXTENSION';
    }
    ...
    /**
     * Event name for calling the mobile platform function.
     * @const {string}
     */
    static get MOBILE_PLATFORM_CALL() {
        return 'MOBILE_PLATFORM_CALL';
    }

    /**
     * Event name when the project has not usable block on the workspace.
     * @const {string}
     */
    static get RUNTIME_ERROR() {
        return 'RUNTIME_ERROR';
    }

    /**
     * Event name when the board has been changed.
     * @const {string}
     */
    static get BOARD_UPDATED() {
        return 'BOARD_UPDATED';
    }

    /**
     * Event name when the mode of the project changed from upload-mode to stage-mode or vice versa.
     * @const {string}
     */
    static get PROJECT_MODE_CHANGED() {
        return 'PROJECT_MODE_CHANGED';
    }

    static get FIELD_IMAGE_SELECT() {
        return 'FIELD_IMAGE_SELECT';
    }

    static get PROJECT_VERSION_ERROR() {
        return 'PROJECT_VERSION_ERROR';
    }

    static get CAPTURE_AUDIO() {
        return 'CAPTURE_AUDIO';
    }

    static get CAPTURE_IMAGE() {
        return 'CAPTURE_IMAGE';
    }

    static get INTERNET_CONNECTION_WARNING() {
        return 'INTERNET_CONNECTION_WARNING';
    }

    static get REQUEST_TIMED_OUT() {
        return 'REQUEST_TIMED_OUT';
    }

    static get DISCONNECT_PORTS() {
        return 'DISCONNECT_PORTS';
    }

    static get TEACHABLE_MACHINE_PREDICT() {
        return 'TEACHABLE_MACHINE_PREDICT';
    }

    static get TEACHABLE_MACHINE_GET_CLASS() {
        return 'TEACHABLE_MACHINE_GET_CLASS';
    }

    static get MODEL_LOADING() {
        return 'MODEL_LOADING';
    }

    static get MODEL_LOADING_FINISHED() {
        return 'MODEL_LOADING_FINISHED';
    }

    static get LOAD_EXTENSION_USING_URL() {
        return 'LOAD_EXTENSION_USING_URL';
    }

    // static get VM_PRE_SERIAL_STORE_STATE_PROCEDURE_STARTED() {
    //     return 'VM_PRE_SERIAL_STORE_STATE_PROCEDURE_STARTED';
    // }

    // static get VM_PRE_SERIAL_STORE_STATE_PROCEDURE_STOPPED() {
    //     return 'VM_PRE_SERIAL_STORE_STATE_PROCEDURE_STOPPED';
    // }

    // static get VM_PRE_SERIAL_STORE_STATE_PROCEDURE_TERMINATED() {
    //     return 'VM_PRE_SERIAL_STORE_STATE_PROCEDURE_TERMINATED';
    // }
    ...
    static get MODEL_FILES_LOCATED() {
        return 'MODEL_FILES_LOCATED';
    }

    static get AFTER_BOARD_SELECTED_UPLOAD_FIRMWARE_MODAL() {
        return 'AFTER_BOARD_SELECTED_UPLOAD_FIRMWARE_MODAL';
    }

    static get STORE_PROCEDURE_ON() {
        return 'STORE_PROCEDURE_ON';
    }

    static get STORE_PROCEDURE_OFF() {
        return 'STORE_PROCEDURE_OFF';
    }
    
    static get STORE_PROCEDURE_WARNING() {
        return 'STORE_PROCEDURE_WARNING';
    }

    static get FIRMWARE_NOT_UPDATED() {
        return 'FIRMWARE_NOT_UPDATED';
    }
    // -----------------------------------------------------------------------------
    // -----------------------------------------------------------------------------
    ...
    _registerExtensionPrimitives(extensionInfo) {
        const categoryInfo = {
            ...
            isBackgroundMenu: extensionInfo.isBackgroundMenu,
            allowRefresh: extensionInfo.allowRefresh
        };

        if (extensionInfo.colour) {
            categoryInfo.color1 = extensionInfo.colour;
            categoryInfo.color2 = extensionInfo.colourSecondary;
            categoryInfo.color3 = extensionInfo.colourTertiary;
        ...
        const found = this._blockInfo.some(el => el.id === categoryInfo.id);
    ...
    _refreshExtensionPrimitives(extensionInfo) {
        let blockInfoList = this._oldBlockInfo;
        this._blockInfo.map(blockInfo => {
            if (!blockInfoList.includes(blockInfo))
                blockInfoList.push(blockInfo);
        });
        const categoryInfo = blockInfoList.find(info => info.id === extensionInfo.id);
        if (categoryInfo) {
    ...
    _fillExtensionCategory(categoryInfo, extensionInfo) {
        ...
        for (const menuName in extensionInfo.menus) {
            if (extensionInfo.menus.hasOwnProperty(menuName)) {
                ...
                // To allow refresh of toolbox when the block which shares the input among the other boards.
                if (categoryInfo.allowRefresh) convertedMenu.json.allowRefresh = true;
        ...
        for (const blockInfo of extensionInfo.blocks) {
            if (blockInfo === '---') {
                ...
                const convertedMessage = this._convertForScratcMessages(blockInfo);
                categoryInfo.blocks.push(convertedMessage);
                continue;
            }
            try {
                ...
                if (convertedBlock.json) {
                    // To allow refresh of toolbox when the block which shares the input among the other boards.
                    if (categoryInfo.allowRefresh) convertedBlock.json.allowRefresh = true;
    ...
    _convertForScratcMessages(blockInfo) {
        const blockXML = `<label text="${blockInfo.message}"></label>`;
        return {
            info: {},
            json: null,
            xml: blockXML
    ...
    getBlocksXML() {
        return this._blockInfo.map(categoryInfo => {
            if (categoryInfo.isBackgroundMenu && categoryInfo.isBackgroundMenu === true) {
                return {};
            }
            ...
            // let isBackgroundMenu = categoryInfo.isBackgroundMenu && categoryInfo.isBackgroundMenu === true;
            // console.log("categoryInfo.id", categoryInfo.id);
            // console.log("isBackgroundMenu", isBackgroundMenu, this.isExtensionForBackground(this.boardSelected, categoryInfo.id));
            if (this.isExtensionForBackground(this.boardSelected, categoryInfo.id)) {
                // this.backgorundExtensionXMLData[returnValue.id] = returnValue.xml;
                // console.log("backgorundExtensionXMLData", this.backgorundExtensionXMLData);
                return {};
            }
            return returnValue;
        });
    ...
    toggleScript(topBlockId, opts) {
        ...
        if (opts.stackClick) {
            ...
            if (opcode === 'event_whenflagclicked') {
                this.sendAnalyticEvent({ eventName: 'green_flag', eventParams: { 'click_type': 'stack_click', 'board_name': this.boardSelected } })
            }
        }
        if (this.hasNotUsableBlockInStack(opts.target, topBlockId)) {
            this.emit(Runtime.RUNTIME_ERROR, this.notUsableBlocks);
            return;
        }
        // Otherwise add it.
    ...
    getWorkspaceTopBlocksTree(optTarget) {
        const allFlagBlocks = {};
        this.allScriptsDo((topBlockId, target) => {
            const blockId = topBlockId;
            const currentBlock = target.blocks.getBlock(blockId);
            const parentBlockNode = new BlockNode({
                opcode: currentBlock.opcode,
                inputs: [],
                fields: currentBlock.fields,
                next: null
            });
            this.buildBlockTree(parentBlockNode, target.blocks, currentBlock);
            // console.log(script);
            // console.log(target);
            if (allFlagBlocks[target.sprite.name] === undefined) {
                allFlagBlocks[target.sprite.name] = [];
            }
            allFlagBlocks[target.sprite.name].push(parentBlockNode);
        }, optTarget);
        return allFlagBlocks;
    }

    buildBlockTree(currentBlockNode, container, currentBlock) {
        const inputs = currentBlock.inputs;
        for (const key of Object.keys(inputs)) {
            const inputBlock = container.getBlock(inputs[key].block);
            const fields = {};
            for (const field of Object.keys(inputBlock.fields)) {
                fields[field] = {
                    name: inputBlock.fields[field].name,
                    value: inputBlock.fields[field].value,
                    range: null
                };
            }
            const newBlockInput = new BlockInput({
                name: inputs[key].name,
                blockNode: {
                    opcode: inputBlock.opcode,
                    inputs: [],
                    fields: fields,
                    next: null
                }
            });
            currentBlockNode.inputs.push(newBlockInput);
            this.buildBlockTree(newBlockInput.blockNode, container, inputBlock);
        }
        if (currentBlock.next !== null) {
            const nextBlock = container.getBlock(currentBlock.next);
            const fields = {};
            for (const field of Object.keys(nextBlock.fields)) {
                fields[field] = {
                    name: nextBlock.fields[field].name,
                    value: nextBlock.fields[field].value,
                    range: null
                };
            }
            const nextBlockNode = new BlockNode({
                opcode: nextBlock.opcode,
                inputs: [],
                fields: fields,
                next: null
            });
            currentBlockNode.next = nextBlockNode;
            this.buildBlockTree(nextBlockNode, container, nextBlock);
        }
        return;
    }
    ...
    startHats(requestedHatOpcode,
        ...
        this.allScriptsByOpcodeDo(requestedHatOpcode, (script, target) => {
            ...
            if (this.hasNotUsableBlockInStack(target, topBlockId)) {
                this.emit(Runtime.RUNTIME_ERROR, this.notUsableBlocks);
                return;
            }
    ...
    verifyCode(lesson) {
        // const targetNodes = this.getWorkspaceTopBlocksTree();
        // const targetJSON = {};
        // for (const target in targetNodes) {
        //     const nodeArray = [];
        //     for (const parentNode in targetNodes[target]){
        //         nodeArray.push(targetNodes[target][parentNode].getBlock());
        //     }
        //     targetJSON[target] = nodeArray;
        // }
        // console.log(JSON.stringify(targetJSON));

        // const lesson = {"Dani":[{"opcode":"event_whenthisspriteclicked","inputs":[],"fields":{},"next":{"opcode":"motion_turnleft","inputs":[{"name":"DEGREES","block":{"opcode":"math_number","inputs":[],"fields":{"NUM":{"name":"NUM","value":"70","range":null}},"next":{}}}],"fields":{},"next":{"opcode":"motion_glidesecstoxy","inputs":[{"name":"SECS","block":{"opcode":"math_number","inputs":[],"fields":{"NUM":{"name":"NUM","value":"1","range":null}},"next":{}}},{"name":"X","block":{"opcode":"math_number","inputs":[],"fields":{"NUM":{"name":"NUM","value":"120","range":null}},"next":{}}},{"name":"Y","block":{"opcode":"math_number","inputs":[],"fields":{"NUM":{"name":"NUM","value":"-60","range":null}},"next":{}}}],"fields":{},"next":{}}}}]};


        const targetNodes = this.getWorkspaceTopBlocksTree();
        const verificationStateMap = {};

        for (const target in lesson) {
            verificationStateMap[target] = [];
            for (const script in lesson[target]) {
                let verificationState;
                verificationStateMap[target].push({
                    script: lesson[target][script].opcode,
                    verificationState: false
                });
                for (const parentNode in targetNodes[target]) {
                    if (targetNodes[target][parentNode].opcode === lesson[target][script].opcode) {
                        verificationState = targetNodes[target][parentNode].verifyCode(lesson[target][script]);
                    }
                    if (verificationState) {
                        verificationStateMap[target][script].verificationState = true;
                        break;
                    }
                }
            }
        ...
        let result = false;
        for (const key in verificationStateMap) {
            console.log(key);
            console.log(verificationStateMap[key]);
            for (const ele in verificationStateMap[key]) {
                if (!verificationStateMap[key][ele].verificationState) {
                    result = false;
                    break;
                }
                result = true;
    ...
    writeToPeripheral(arg, type, mode) {
        const data = SerialCom.sendPackage(arg, type, mode);
        // if (this.vmPreSerialStoreProcedureState === VMPreSerialStoreProcedureState.STARTED) {
        //     // console.log("serialWrite QUEUE");
        //     this.serialWriteQueue.push(data);
        // }
        // else {
            this.writeToPeripheralUsingPackage(data);
        // }
    }

    writeToPeripheralUsingPackage(data, shouldBeParsed = true) {
        console.log(`from write: ${data} ${shouldBeParsed} | writeToPeripheralUsingPackage runtime.js`);
        this.emit('SERIAL_WRITE', data, shouldBeParsed);
    }

    serialRead(data, isVMStoreProcess) {
        console.log(`from read: ${data} | serialRead runtime.js`);
        // console.log("serialRead FROM RUNTIME isVMStoreProcess", isVMStoreProcess);
        if (isVMStoreProcess) {
            this.readForVMPreStore(data);
        } else {
            this.serialValue = data;
            this.emit('READ_FROM_PERIPHERAL', data);
        }
    }

    redoUndo(flag) {
        // flag = true for redo, false = undo
        this.emit('redoUndo', flag);
    }

    zoomInOut(type) {
        // type = -1 zoom out, type = 1 for zoom in
        this.emit('zoomInOut', type);
    }
     ...
     * @param {boolean} block Dragged block
     */
    ...
    requestAddMonitor(monitor) {
        console.log("requestAddMonitor", monitor);
    ...
    /**
     * Handles input focus event and opens dialog for user to enter value
     * @param {HTMLInputElement} inputRef
     */
    handleInputFocus(inputRef, type, placeholder, handlerName, currValue) {
        this.currentInputRef = inputRef;
        if (this.currentInputRef) {
            this.currentInputRef.blur();
            this.emit(Runtime.MOBILE_PLATFORM_CALL, "openUserInput", type, placeholder, handlerName, currValue);
        }
    }
     ...
     */
    setInputValue(data) {
        this.currentInputRef.value = data.value;
        this.currentInputRef.dispatchEvent(new Event('keyup'))
    }
    ...
    /**
     * Checks if the project has any 'not usable' block.
     * @returns {boolean} false if the list of not usable is empty.
     */
    doProjectHasNotUsableBlock() {
        return this.notUsableBlocks.length && this.notUsableBlocks.length > 0;
    }
    /**
     * Adds the block to the list of not usable blocks, to make a check when the block is being executed.
     * @param {String} blockId Id of the block which is not usable.
     */
    addBlockToNotUsableBlockList(blockId) {
        this.notUsableBlocks.push(blockId);
    }
    /**
     * Removes the block from the list of not usable blocks.
     * @param {String} blockId Id of the block which is not usable.
     */
    removeBlockToNotUsableBlockList(blockId) {
        if (this.notUsableBlocks.includes(blockId)) {
            this.notUsableBlocks = this.notUsableBlocks.filter((blockIdElement) => {
                return blockIdElement != blockId;
            });
        }
    }

    /**
     * Resets the list of not usable block list.
     */
    resetNotUsableBlockList() {
        this.notUsableBlocks = [];
    }

    /**
     * Checks if the blocks of the editing target has not usable block in the block tree of the top block.
     * @param {String} target target ID for target to run script on. If not supplied, uses editing target.
     * @param {String} topBlockId Id of the top block.
     */
    hasNotUsableBlockInStack(target, topBlockId) {
        if (!topBlockId || !target.blocks.getBlock(topBlockId)) return false;
        let topBlock;
        do {
            if (this.notUsableBlocks.includes(topBlockId)) {
                return true;
            }
            topBlock = target.blocks.getBlock(topBlockId);
    ...
    /**
     * Updates the board when user selects the board.
     * @param {String} board the board to be updated.
     */
    setBoard(board) {
        this.boardSelected = board;
        this.emit(Runtime.BOARD_UPDATED, board, true /**  True, if need to show check the pins warning modal */);
    }

    /**
     * Merges the list of the blocks which are from common extension but are specific to the each board with boardSpecificExtraBlocksFromCommonExtensionObj
     * @param {Object} extraBlocksFromCommonExtensionObj
     */
    megerBoardSpecificExtraBlocksFromCommonExtensionDict(extraBlocksFromCommonExtensionObj) {
        for (let boardIndex in extraBlocksFromCommonExtensionObj) {
            let blockOpcodeList = extraBlocksFromCommonExtensionObj[boardIndex];
            if (!this.boardSpecificExtraBlocksFromCommonExtensionObj.hasOwnProperty(boardIndex)) {
                this.boardSpecificExtraBlocksFromCommonExtensionObj[boardIndex] = [];
            }
            for (let blockOpcodeIndex in blockOpcodeList) {
                if (!this.boardSpecificExtraBlocksFromCommonExtensionObj[boardIndex].includes(blockOpcodeList[blockOpcodeIndex]))
                    this.boardSpecificExtraBlocksFromCommonExtensionObj[boardIndex].push(blockOpcodeList[blockOpcodeIndex]);
            }
        }
        // console.log("registered", this.boardSpecificExtraBlocksFromCommonExtensionObj);
    }

    /**
     * Merges the list of the blocks which are from common extension and is common to other boards with boardSpecificBlocksFromCommonExtensionObj
     * @param {Object} blocksFromCommonExtensionObj
     */
    megerBoardSpecificBlocksFromCommonExtensionDict(blocksFromCommonExtensionObj) {
        for (let boardIndex in blocksFromCommonExtensionObj) {
            let blockOpcodeList = blocksFromCommonExtensionObj[boardIndex];
            if (!this.boardSpecificBlocksFromCommonExtensionObj.hasOwnProperty(boardIndex)) {
                this.boardSpecificBlocksFromCommonExtensionObj[boardIndex] = [];
            }
            for (let blockOpcodeIndex in blockOpcodeList) {
                if (!this.boardSpecificBlocksFromCommonExtensionObj[boardIndex].includes(blockOpcodeList[blockOpcodeIndex]))
                    this.boardSpecificBlocksFromCommonExtensionObj[boardIndex].push(blockOpcodeList[blockOpcodeIndex]);
            }
        }
        // console.log("registered common blocks", this.boardSpecificBlocksFromCommonExtensionObj);
    }

    /**
     * Registers the common extensions to belonging boards.
     */
    registerCommonExtensions() {
        Object.keys(this.defaultHardwareExtensions).map((board) => {
            this.commonHardwareExtensions.map((commonExtension) => {
                if (this.defaultHardwareExtensions[board].includes(commonExtension)) {
                    this.manageCommonExtensionListAmongBoards(board, commonExtension);
                };
            });
        });
    }
    ...
    /**
     * Manages the list of common extension among the boards.
     * @param {String} board for which, we are going to register common extension.
     * @param {*} commonExtensions the common extension being registered.
     */
    manageCommonExtensionListAmongBoards(board, commonExtensions) {
        if (typeof commonExtensions !== 'object' && typeof commonExtensions === 'string') {
            commonExtensions = [commonExtensions];
        }
        commonExtensions.map((commonExtensionObj) => {
            if (this.commonExtensionAmongBoards.hasOwnProperty(board)) {
                if (!this.commonExtensionAmongBoards[board].includes(commonExtensionObj)) {
                    this.commonExtensionAmongBoards[board].push(commonExtensionObj);
                }
            } else {
                this.commonExtensionAmongBoards[board] = [];
                this.commonExtensionAmongBoards[board].push(commonExtensionObj);
            }
        });
    }
    ...
    /**
     * Emit an event to indicate that project file being loaded is from newer version.
     */
    emitProjectVersionError() {
        this.emit(Runtime.PROJECT_VERSION_ERROR);
    }

    /**
    * Adds extension Id to the list of loaded extensionIds from extensionLibrary, to make load it again when board gets changed.
    * @param {String} extensionId Id of the extension which is being loaded.
    */
    addExtensionIdToLoadedExtension(extensionId) {
        this.commonExtensionLoadedFromExtensionLibrary.push(extensionId);
    }

    /**
     * Assigns list of extensions to default extensions.
     * @param {Object} defaultExtensions Default extensions provided.
     */
    assignDefaultExtensions(defaultExtensions) {
        this.defaultExtensions = defaultExtensions;
    }

    /**
     * Emit an event to indicate that project mode has been changed.
     */
    emitProjectModeChanged() {
        this.emit(Runtime.PROJECT_MODE_CHANGED);
    }

    /**
     * Emit an event to indicate that project should disconnect all connected ports.
     */
    emitDisconnectPorts() {
     ...
     * @param {String} platform string containing os.
     */
    ...
    /**
     * Sets the userId when user gets logged in or gets logged out
     * @param {String} userId user id of the user logged in
     */
    registerUser(userId) {
        this.userId = userId;
    }
    ...
    /**
     * To call a functtion on AI server
     * @param {string} param0 url where to make a request
     * @param {string} param1 params of the request
     * @param {string} param2 body of the request
     */
    doAIServerCall({ url, params, data, contentType }) {
        // Do not change the content-type in the request, otherwise this request have CORS policy issues.
        // this.checkInternetConnection().then(isOnline => {
        //     if (!isOnline) return new Promise((resolve, reject) => {
        //         reject("Internet connection disconnected!");
        //     });
        return new Promise((resolve, reject) => {
            axios({
                ...
                headers: {
                    ...
                    'pictoblox-token': this.userId ? this.userId : ""
                },
                params: params,
                data: data
            }).then(resp => {
                ...
                if (exception.response) {
                    if (exception.response.status === 401) {
                        this.emit(Runtime.SIGN_IN_REQUIRED_ERROR);
    ...
    /**
     * To check if the extension requires sign-in before being used
     * @param {string} extensionURL extension URL
     * @returns {boolean} true if it required sign-in
     */
    isSignInRequiredForThisExtension(extensionURL) {
        return this.signInRequiredExtensions.includes(extensionURL);
    }

    /**
     * Will be called from the functions from
     * @param {string} extensionName the name of the extension which has called this function
     */
    checkSessionExists(extensionName) {
        if (typeof this.userId === 'undefined' || this.userId == null) {
            this.emit(Runtime.SIGN_IN_REQUIRED_ERROR, extensionName);
            return false;
        }
        return true;
    }
     ...
     * @returns {Promise} true if connected to the internet.
     */
    ...
    /**
     * Checks if the extensionUrl belongs to Scratch or not
     * @param {string} extensionUrl the name of the extension
     */
    checkIsScratchExtension(extensionUrl) {
        return this.scratchExtensions.includes(extensionUrl);
    }

    /**
     * The extensionUrl to be loaded
     * @param {string} extensionUrl the name of the extension
     * @param {string} extensionId the id of the extension
     */
    loadExtensionUsingURL(extensionUrl, extensionId) {
        return this.emit(Runtime.LOAD_EXTENSION_USING_URL, extensionUrl, extensionId);
    }

    /**
     * Checks if to show notice when this extension loaded
     * @param {string} extensionUrl the name of the extension
     */
    checkIfToShowNotice(extensionUrl) {
        return this.showNoticeOnExtensionLoad.includes(extensionUrl);
    }

    /**
     * Registers how many times AI services has been used by the user
     * @param {string} aiServiceCount the times user has user AI Services
     */
    registerAIServiceCount(aiServiceCount) {
        this.aiServiceCount = aiServiceCount;
    }
     ...
     */
    stopRunningScripts() {
        // console.log("stopRunningScripts");
        for (let i = 0; i < this.threads.length; i++) {
            this._stopThread(this.threads[i]);
        }
    }
     ...
     */
    setVideoInputDevice(videoInputDevicePosition) {
        this.videoInputDevicePosition = videoInputDevicePosition;
    }

    /**
     * VideoProvider Instace Container
     */
    addVideoProviderInstance(videoProvider) {
        this.videoProviderContainer.push(videoProvider);
    }

    stopAllVideoProvideInstance() {
        console.log("stopAllVideoProvideInstance", this.videoProviderContainer.length);
        for (let instanceIndex = 0; instanceIndex < this.videoProviderContainer.length; instanceIndex++) {
            setTimeout(() => {
                this.videoProviderContainer[instanceIndex].disableVideo();
            });
        }
    }
     ...
     */
    checkIfBoardAllowedForVMPreSerialStoreProcedure(board) {
        return this.vmPreSerialStoreProcedureAllowedBoards.includes(board);
    }

    initialiseVMPreSerialStore() {
        if (typeof this.VMPreSerialStore[this.boardSelected] == 'undefined') {
            console.log("initialiseVMPreSerialStore", this.boardSelected);
            this.VMPreSerialStore[this.boardSelected] = {};
            this.VMPreSerialStore[this.boardSelected]['firwareUploaded'] = false;
            this.isVMStoreRuning = true;
        }
    }

    /**
     * Starts the VMPreSerialStore procedure to serial read and write for pinSelectedForVMPreSerialStore pin
     */
    startVMPreStoreProcedure() {
        // console.log("startVMPreSerialStoreProcedure")

        // this.vmPreSerialStoreProcedureState = VMPreSerialStoreProcedureState.STARTED;
        // this.serialWriteQueueProcedureState = VMPreSerialStoreProcedureState.STARTED;

        this.initialiseVMPreSerialStore();
        this.stepVMPreStoreThread(true);
        // if (this.serialWriteQueueInterval) {
        //     clearInterval(this.serialWriteQueueInterval);
        // }
        // this.serialWriteQueueInterval = setInterval(() => {
        //     this.stepSerialWriteQueue(true);
        // }, this.serialWriteQueueIntervalValue);
    }

    stepVMPreStoreThread(isFirst = false) {
        // if (!this.checkIfBoardAllowedForVMPreSerialStoreProcedure(this.boardSelected)) {
        //     console.error("Board is not allowed for VMPreSerialStoreProcedure");
        //     this.terminateVMPreSerialStoreProcedure(true);
        //     return;
        // }
        if (!this.getFirmwareUploadedStatus()) {
            console.error("Firware is not uploaded in peripheral!");
            // this.terminateVMPreSerialStoreProcedure(true);
            return;
        }
        // if (isFirst || this.vmPreSerialStoreProcedureState === VMPreSerialStoreProcedureState.STOPPED) {
        //     console.log("VMPreSerialStoreProcedureExecuted COUNT: ", this.timesVMPreSerialStoreProcedureExecuted);
            // this.timesVMPreSerialStoreProcedureExecuted++;
            // this.vmPreSerialStoreProcedureState = VMPreSerialStoreProcedureState.STARTED;
            // this.writeToPeripheralForVMStore();
        // }
     }

    // /**
    //  * Serial Write for the selected pin in pinSelectedForVMPreSerialStore for VMPreSerialStore
    //  */
    // writeToPeripheralForVMStore() {
    //     // console.log("serialWriteForVMStore", Date.now());
    //     const data = SerialCom.sendPackage([1], 170, 1);
    //     this.serialWriteUsingPackage(data, false);
    //     // const data = SerialCom.sendPackage(dataBuffer, 30, 1);
    //     // this.emit('serialWrite', data);
    // }

    /**
     * Serial Read for the selected pin in pinSelectedForVMPreSerialStore for VMPreSerialStore
     * This will store the data received
     */
    readForVMPreStore(data) {
        // console.log("readForVMPreStore", Date.now());
        this.initialiseVMPreSerialStore();
        // console.log("DATA RECEIVED", data);
        this.VMPreSerialStore[this.boardSelected].data = data;
    }

    /**
     * Get stored data for corresponding pin
     * @param {number} pinSelectedForVMPreSerialStore the pin for which data are required
     * @returns {object}
     */
    getVMPreStoreData() {
        // Only Quarky is allowed
        // if (this.boardSelected !== boards.Quarky) return;
        return this.VMPreSerialStore[this.boardSelected].data;
    }

    /**
     * Stops and starts again the VMPreSerialStore procedure
     */
    // restartVMPreSerialStoreProcedure() {
    //     if (!this.isVMPreSerialStoreProcedureRunning()) {
    //         return;
    //     }
    //     this.vmPreSerialStoreProcedureState = VMPreSerialStoreProcedureState.STOPPED;
    //     this.emit(Runtime.VM_PRE_SERIAL_STORE_STATE_PROCEDURE_STOPPED);

    //     // If there is another timeout, clear that
    //     if (this.vmPreSerialStoreRestartTimeout)
    //         clearTimeout(this.vmPreSerialStoreRestartTimeout);

    //     this.vmPreSerialStoreRestartTimeout = setTimeout(() => {
    //         this.stepVMPreStoreThread();
    //     }, this.vmPreSerialStoreRestartTimeoutValue);
    // }

    /**
     * Terminates the VMPreSerialStore procedure
     */
    // terminateVMPreSerialStoreProcedure(force = false) {
    //     console.log("terminateVMPreSerialStoreProcedure");
    //     if (force || this.vmPreSerialStoreRestartTimeout ||
    //         this.isVMPreSerialStoreProcedureRunning()) {
    //         if (this.vmPreSerialStoreRestartTimeout) {
    //             clearTimeout(this.vmPreSerialStoreRestartTimeout);
    //         }
    //         this.vmPreSerialStoreProcedureState = VMPreSerialStoreProcedureState.TERMINATED;
    //         this.emit(Runtime.VM_PRE_SERIAL_STORE_STATE_PROCEDURE_TERMINATED);
    //         this.firmwareUploadReport(false, false);
    //         this.terminateSerialWriteQueue(force);
    //     } else {
    //         console.error("VMPreSerialStore Procedure has already been terminated");
    //     }
    // }

    // stepSerialWriteQueue(isFirst = false) {
        // console.log("stepSerialWriteQueue");
        // if (this.serialWriteQueueProcedureState === VMPreSerialStoreProcedureState.TERMINATED) {
        //     console.error("VMPreSerialStore Procedure had been terminated");
        //     return;
        // }
        // if (this.serialWriteQueue.length > 0) {
            // console.log("this.serialWriteQueue", this.serialWriteQueue);
            // console.log("serialWriteQueueProcedureState STARTED");
            // this.serialWriteQueueProcedureState = VMPreSerialStoreProcedureState.STARTED;
            // const data = this.serialWriteQueue.shift();
            // console.log("SERVING....", data);
            // this.serialWriteUsingPackage(data);
        // } else if (this.serialWriteQueueProcedureState !== VMPreSerialStoreProcedureState.STOPPED) {
            // console.log("serialWriteQueueProcedureState STOPPED");
            // this.serialWriteQueueProcedureState = VMPreSerialStoreProcedureState.STOPPED;
        // }
    // }
     ...
     * Terminates the VMPreSerialStore procedure
     */
    // terminateSerialWriteQueue(force = false) {
    //     console.log("terminateSerialWriteQueue");
    //     if (force || this.serialWriteQueueInterval ||
    //         this.serialWriteQueueProcedureState !== VMPreSerialStoreProcedureState.TERMINATED) {
    //         if (this.serialWriteQueueInterval) {
    //             clearInterval(this.serialWriteQueueInterval);
    //         }
    ...
    /**
     * Stores the status of firmware uploading process in VM Pre Serial Store
     * @param {boolean} status represents if firware has been uploaded in peripheral
     */
    firmwareUploadReport(status, version = 2, sholdInteractWithVMPreSerialStoreProcedure = true) {
        // console.log("fimrwareUploadReport", status, sholdInteractWithVMPreSerialStoreProcedure);
        this.initialiseVMPreSerialStore();
        this.VMPreSerialStore[this.boardSelected]['firwareUploaded'] = status;
        console.log(`setFirmwareVersion(version) called: ${version} | runtime.js`);
        this.firmwareVer = version;
        if(this.firmwareVer == 1 && this.boardSelected == boards.Quarky)
        {
            console.log("Firmware version ");
            this.emit(Runtime.FIRMWARE_NOT_UPDATED);
        }
        // if (status && sholdInteractWithVMPreSerialStoreProcedure) this.startVMPreSerialStoreProcedure();
        // if (!status
        //     && sholdInteractWithVMPreSerialStoreProcedure
        //     && this.isVMPreSerialStoreProcedureRunning())
        //     this.terminateVMPreSerialStoreProcedure();
    }

    /**
     * To get the status of firware uploaded in the current peripheral
     * @returns {boolean} status represents if firware has been uploaded in peripheral
     */
    getFirmwareUploadedStatus() {
        return this.VMPreSerialStore[this.boardSelected]['firwareUploaded'];
    }

    // isVMPreSerialStoreProcedureRunning() {
    //     return this.vmPreSerialStoreProcedureState !== VMPreSerialStoreProcedureState.TERMINATED;
    // }

    /**
     * Check if the extensionURL should be load in background or not for the given board
     * @param {*} board the board with respect to this extensionURL is being checked
     * @param {*} extensionURL URL of the extension
     */
    isExtensionForBackground(board, extensionURL) {
        return (this.backgorundHardwareExtensions.hasOwnProperty(board) && this.backgorundHardwareExtensions[board].includes(extensionURL))
            || this.backgorundHardwareExtensions.hasOwnProperty('all') && this.backgorundHardwareExtensions['all'].includes(extensionURL);
    }

    emitModelLoading(modelType, modelLoadingId = "") {
        console.log("emitModelLoading", modelType, modelLoadingId);
        this.modelLoadingType = modelType;
        this.modelLoadingId = modelLoadingId;
        this.emit(Runtime.MODEL_LOADING, modelType);
    }

    emitModelFilesLocated(modelPath, isAbort = false) {
        console.log("MODEL_FILES_LOCATED", this.modelLoadingType, modelPath, isAbort);
        this.emit("MODEL_FILES_LOCATED" + this.modelLoadingId, modelPath, isAbort);
    }

    /**
     * Checks if the selected board is available for upload firmware or not
    */
    isAvailableForUploadFirmware(boardName) {
        return this.uploadFirmwareAvailableBoards.includes(boardName);
    }

    /**
     * Is data available for the specific board
     * @param {string} board check for the board
     */
    isVMPreStoreDataAvailable() {
        return this.VMPreSerialStore.hasOwnProperty(this.boardSelected) &&
            this.VMPreSerialStore[this.boardSelected].hasOwnProperty("data");
    }

    /**
     * Checks if any extension from dataConsentNeededForExtensions has been in project.
     * Extension from dataConsentNeededForExtensions needs user consent.
     */
    isUserContentNeeded() {
        // Gets the extensions which are loaded manually
        let doUserContentNeeded = false;
        this._blockInfo.forEach((blockInfo) => {
            if (this.dataConsentNeededForExtensions.includes(blockInfo.id)) {
                doUserContentNeeded = true;
            }
        });
        return doUserContentNeeded;
    }
