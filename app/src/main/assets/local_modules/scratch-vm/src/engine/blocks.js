...
class Blocks {
    ...
    /**
     * Provide an object with list of blocks in this workspace
     * @return {?object} Blocks in the workspace.
     */
    getBlocks() {
        return this._blocks;
    }
    ...
    /**
     * Resets the collection of blocks when a new project is created
     */
    resetBlocks() {
        this._blocks = {};
    }
    ...
    blocklyListen(e) {
        ...
        if (e.element === 'selected' && e.group === '' && e.newValue) {
            this.runtime.sendAnalyticEvent({
                eventName: 'block_created',
                eventParams: {
                    value: e.newValue
                }
            });
            // Add extension name here to log analytics when its block gets created.
            // if (
            //     e.newValue.startsWith('evive') ||
            //     e.newValue.startsWith('arduino') ||
            //     e.newValue.startsWith('humanoidRobot') ||
            //     e.newValue.startsWith('advanceSensor') ||
            //     e.newValue.startsWith('roboticArm') ||
            //     e.newValue.startsWith('stepperMotor') ||
            //     e.newValue.startsWith('neuralNetwork') ||
            //     e.newValue.startsWith('csvFormatter') ||
            //     e.newValue.startsWith('posenet') ||
            //     e.newValue.startsWith('objectDetection') ||
            //     e.newValue.startsWith('faceDetection') ||
            //     e.newValue.startsWith('qrCodeScanner') ||
            //     e.newValue.startsWith('customObjectDetection') ||
            //     e.newValue.startsWith('physcisEngine') ||
            //     e.newValue.startsWith('naturalLanguageProcessing')
            // ) {
        ...
        switch (e.type) {
            ...
            case 'dragOutside':
                ...
                break;
            case 'endDrag':
                this.runtime.emitBlockDragUpdate(false /* areBlocksOverGui */, this._blocks[e.blockId]);
            ...
            case 'comment_delete':
                ...
                break;
            case 'input_focus':
                let type = e.name == "NUM" ? "float" : 'string';
                let currValue = e.inputRef.value;
                this.runtime.handleInputFocus(e.inputRef, type, 'Enter Field Value', 'Field_Input', currValue);
                break;
            case 'field_image_select':
                this.runtime.emit('FIELD_IMAGE_SELECT', e.field);
                break;
        ...
        switch (args.element) {
            case 'field':
                ...
                if (args.name === 'VARIABLE' || args.name === 'LIST' ||
                    args.name === 'BROADCAST_OPTION' || args.name === "IMAGE_OPTION") {
