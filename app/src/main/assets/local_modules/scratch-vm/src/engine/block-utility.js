...
class BlockUtility {
     ...
     * Implemented for hardware implementation of C-type blocks. 
     */
    getThisBlock () {
        return this.thread.peekStack();
    }

    color565 (COLOR) {
        const color = COLOR.substr(1);
        const rgb = color.match(/.{2}/g).map(ele => parseInt(ele, 16));
        return ((rgb[0] & 0xF8) << 8) | ((rgb[1] & 0xFC) << 3) | (rgb[2] >> 3);
    }

    short2array (byte) {
        return [(byte & 0x00FF), (byte & 0xFF00) >> 8];
    }

    long2array (byte) {
        return [(this.short2array(byte & 0x0000FFFF)), (this.short2array(byte & 0xFFFF0000))];
    }

    detectNumOrString (value) {
        const num = (isNaN(value) ? value.replace(/['"]+/g, '') : value);
        if (isNaN(num)) {
            return value;
        }
        return num;
    ...
    setDelay (delayTime){
        return new Promise(resolve => {
            setTimeout(resolve, delayTime * 1);
        });
    }
    ...
    rgb565 (image, size){
        let rgb = new Uint16Array(size.width * size.height);
        for(var i=0; i < size.width * size.height * 4; i+=4){
            rgb[i/4] = ((image[i+2] & 0b11111000) << 8) | ((image[i+1] & 0b11111100) << 3) | (image[i] >>> 3);
        }
        return rgb;
